    ...
    const std::string& sectionName) 
{
    ...
    // GROMACS acepta encabezados de sección con o sin espacios internos
    // ("[atomtypes]" y "[ atomtypes ]" son equivalentes). Normalizamos
    // ambas cadenas quitando todos los espacios antes de comparar.
    std::string target = sectionName;
    ...
    for (size_t i = 0; i < allLines.size(); ++i) {
        ...
        if (!allLines[i].empty()) {
            std::string candidate = allLines[i];
...
std::unique_ptr<AtomType> GMXParser::parseAtomTypeLine(const std::string& line) {
    ...
    // Formato estandar [atomtypes] de GROMACS: 6 campos
    //   name mass charge ptype sigma epsilon
    // (NOTA: la version previa leia un atomNum ficticio como 7mo campo,
    //  lo que corrompia mass por el residuo ".0" tras un entero. Corregido.)
    if (!(ss >> name >> mass >> charge >> ptype >> sigma >> epsilon)) {
        return nullptr; // No pudo parsear
    }

    // Crear y retornar nuevo AtomType (RAII, sin new/delete manual)
    // Nota: AtomType es un agregado (config.h) sin constructor, por lo que
    // make_unique con parentesis no compila en C++17; usamos brace-init.
    return std::unique_ptr<AtomType>(new AtomType{name, mass, charge, sigma, epsilon});
}

std::unique_ptr<Bond> GMXParser::parseBondLine(const std::string& line) {
    std::stringstream ss(line);
    ...
    return std::unique_ptr<Bond>(new Bond{atom1 - 1, atom2 - 1, r0, kb});
}
...
GMXTopology GMXParser::parseTopologyFile(const std::string& filepath) {
    ...
    for (const auto& line : atomTypeLines) {
        auto atom = parseAtomTypeLine(line);
        if (atom) {
