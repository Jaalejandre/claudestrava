...
// ========== EWALD SETUP ==========
ewald_setup(cfg, cfg.rcut_coulomb, 1e-5, 1e-5);
 ...
 double* d_charge;
    cudaMalloc(&d_charge, bytes);
    cfg.d_charge = d_charge;
    ...
    cudaMemcpy(d_charge, cfg.charge.data(), bytes, cudaMemcpyHostToDevice);
    ...
    // Initialize CUDA Streams (NEW)
    fprintf(stderr, "[INIT] Initializing CUDA Streams...\n");
    cudaStreamsInit();
    fprintf(stderr, "[INIT] CUDA Streams ready\n");
    ...
    for (int step = 0; step < cfg.nsteps; step++) {
        ...
        // CRITICAL FIX v1.2.1: Synchronize streams to prevent race conditions
        // Without this, host loop launches kernels faster than GPU can execute
        ...
        if (step % 20 == 0) {
            ...
            // BUGFIX v1.2: Convert KE from amu*(nm/ps)^2 to Joules before temperature calc
            double KE_joules = KE * 1.66054e-23;  // Conversion factor
            double T_current = (2.0 / 3.0) * KE_joules / (cfg.natoms * 1.380649e-23);
    ...
    // Cleanup CUDA Streams (NEW)
    fprintf(stderr, "[CLEANUP] Destroying CUDA Streams...\n");
    cudaStreamsDestroy();
    fprintf(stderr, "[CLEANUP] CUDA Streams cleaned up\n");
    ...
    std::cout << "========================================\n"
              ...
              << "========================================\n\n";
 // Cleanup kwald GPU resources
 kwald_free_cuda_stream();
