...
> **Cambio #3 INTENTADO Y REVERTIDO (2026-09-04 noche)** — kernel por átomo-i sin atomicAdd de fuerza en LJ. Física correcta (Total −89.05648±0.01240 dentro de 1σ) pero **≈4.1× MÁS LENTO** (8m32.766s vs 2:04.79 referencia): al construir la lista de adyacencia simétrica autocontenida en cada llamada, el trabajo por-par pasó de `1×npares` a `~4×npares` (dos pasadas extra de construcción CSR + cómputo de fuerza duplicado en ambos extremos del par). Revertido a `6c8db7d`, sin commit. Detalle honesto en `07 Iteration Logs/(C) 2026-09-04 Intento fallido - kernel por atomo-i sin atomicAdd de fuerza (cambio 3).md`. **No se reintenta esta sesión** — requeriría persistir la lista CSR en `lista_linkcell_cuda.cu` (tocar interfaz Fortran) para no ser rentable, alcance mayor al autorizado ahora.
> **Siguiente en la cola:** #5 (revisar `construir_pares`) → #6 (`pow()`→multiplicación, sobre el kernel pair-based validado en `6c8db7d`, NO sobre el experimento fallido de #3) → #7 (OpenMP CPU, aún sin tocar, el bucle sigue en 1 core).
