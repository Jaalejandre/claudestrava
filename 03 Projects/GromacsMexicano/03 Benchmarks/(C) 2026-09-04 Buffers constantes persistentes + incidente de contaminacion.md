...
**Lección para el proyecto:** antes de aceptar cualquier benchmark, verificar `w`, `ps aux`, y `nvidia-smi --query-compute-apps` para confirmar que **nada más** está usando CPU/GPU en CT 901 — el contenedor se comparte con DM UAMI (código) y potencialmente con sesiones interactivas del usuario o agentes en background que no siempre terminan limpio.
