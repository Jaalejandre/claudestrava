...
| GPU                | **RTX 5070 Ti pasada por dispositivo** (compartida con CT 901, no exclusiva) — ver sección GPU abajo                             |
...
- **Verificar:** `nvidia-smi` (debe mostrar la RTX 5070 Ti) y `nvcc --version`
- **Fix aplicado (Ubuntu 26.04 + CUDA 13.0):** el glibc de 26.04 es más nuevo de lo que CUDA 13.0 espera — `rsqrt`/`rsqrtf` chocan en especificación de excepción al compilar. Se parcheó `/usr/local/cuda-13.0/targets/x86_64-linux/include/crt/math_functions.h` agregando `noexcept(true)` a esas dos declaraciones (backup en `math_functions.h.bak`). Si se reinstala el Toolkit, hay que re-aplicar este parche o la compilación con `nvcc` falla con "exception specification is incompatible".
- **Config del contenedor:** `/etc/pve/lxc/109.conf` en el host tiene `dev0`-`dev5` (nodos `/dev/nvidia*`) + `lxc.cgroup2.devices.allow: a` — igual que CT 901.
- **Disco:** se agrandó de 30GB a 60GB para tener espacio para el Toolkit (~5GB) + margen.
