...
- **Plugin `ponytail`** (Claude Code, instalado 2026-09-08 en Mac + CT 109 + CT 901) — "lazy senior dev": fuerza la solución más corta que funcione (YAGNI, stdlib primero, sin abstracciones no pedidas). Comandos: `/ponytail [lite|full|ultra|off]`, `/ponytail-review` (revisar diff por over-engineering), `/ponytail-audit`. ~1k tok siempre-on. Quitar: `claude plugin uninstall ponytail@ponytail`.
- **Agente local para prototipos** — Ollama en CT 103 (GPU, modelos en `nvme-fast`): `qwen2.5-coder:14b` + `gpt-oss:latest`. Para prototipos web simples y desechables, Claude orquesta y revisa, el modelo local genera. Skill [[prototipo-local.md]], carpeta `03 Projects/Prototipos/`. **Nuevo (2026-09-08):** el modelo local hace TODO vía `harness.py` en CT 109 (`/project/prototipos/harness.py`, copia versionada en el vault) — genera, testea (compile + probes HTTP a todas las rutas + integridad de archivos con regla `## Archivos modificables`), se corrige solo (loop 1–6 iteraciones) y levanta el servicio con URL. Memoria de aprendizaje del modelo local en `/project/prototipos/error_log.json`. **Proyecto vivo:** `airbnb-admin` (dashboard limpieza/inventario/incidencias/bloqueos iCal) en `http://192.168.0.64:8877/`, funcional v2; rediseño solo-CSS pendiente (spec v4).
